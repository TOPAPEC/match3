...
export class FieldManagerOptimized extends PIXI.Container {
    ...
    printFieldSize() {
        ...
        console.log("FIELD LEN", len);
    }

    constructor(screenWidth, screenHeight, spritesheet, x, y, fieldWidth, fieldHeight, gemTypes, background, playerData) {
        super();
        ...
        this.x = x;
        this.y = y;
        this.tsize = this.spritesheet.textures[`gem1.png`].width / 2;
        this.field = new Array(fieldWidth).fill(0).map(() => new Array(fieldHeight).fill(0));;
        this.printFieldSize();
    ...
    createGem(tid, gemX, gemY) {
        ...
        if (tid === -1) {
            ...
            texture = new PIXI.Sprite(this.spritesheet.textures[`gem${tid}`]);
        }
    ...
    async init() {
        ...
        this.backgroundRectangle.x = -this.backgroundRectangle.getGlobalPosition().x;
        this.backgroundRectangle.y = -this.backgroundRectangle.getGlobalPosition().y;
        ...
        const textureExample = this.spritesheet.textures["gem1.png"];
        this.textureHeight = textureExample.height / 2;
        this.textureWidth = textureExample.width / 2;
        let cnt = 0;
        this.printFieldSize();
    ...
    loadLevel(levelSchema) {
        this.skipProcess = true;
        let cnt = 0;
        this.printFieldSize();
        for (let listof_child of this.field) {
            for (let child of listof_child) {
                cnt += 1;
                console.log("Child", cnt, child);
                if (typeof(child) != "number") {
                    child.destroyGem();
                }
            }
        }
        this.field = new Array(levelSchema.length).fill(0).map(() => new Array(levelSchema[0].length).fill(0));;
        this.fieldWidth = levelSchema.length;
        this.fieldHeight = levelSchema[0].length;
        for (let i = 0; i < levelSchema.length; i++) {
            for (let j = 0; j < levelSchema[0].length; j++) {
                const gemX = i * this.tsize;
                const gemY = j * this.tsize;
                let tid = levelSchema[i][j];
                if (levelSchema[i][j] !== -1) {
                    tid = this.generateTid(i, j);
                }
                this.field[i][j] = this.createGem(tid, gemX, gemY);
    ...
    process(delta) {
        ...
        if (!anyoneMoved) {
            ...
            if (this.destroyQueue.length > 0 || this.children.length < this.fieldWidth * this.fieldHeight) {
                ...
                if (destroyedGems > 0) {
                    this.playerData.updateCurrentScore(this.playerData.getCurrentScore() + destroyedGems);
                    console.log("CHECKING NULLS IN LOAD")
                    for (let i = 0; i < this.fieldWidth; i++) {
                        for (let j = 0; j < this.fieldHeight; j++) {
                            if (this.field[i][j] === null) {
                                console.log("NULL!", i, j);
                            }
                        }
                    }
    ...
    crossExplosion(i, j) {
        ...
        for (let k = 0; k < this.fieldWidth; k++) {
            console.log("DESTROYING", k, j);
            if (k === i || this.field[k][j] === null || this.isWall(k, j)) {
                continue;
            }
            cnt++;
        ...
        for (let k = 0; k < this.fieldHeight; k++) {
            console.log("DESTROYING", i, k);
            if (k === j || this.field[i][k] === null || this.isWall(i, k)) {
                continue;
            }
            cnt++;
    ...
    emptyDestroyQueue(destroy=false) {
        console.log("DESTROYING", this.destroyQueue);
